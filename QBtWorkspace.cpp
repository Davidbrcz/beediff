...
/*------- local constants:
-------------------------------------------------------------------*/
const char *const QBtWorkspace::NOT_FILE =
    QT_TR_NOOP("The file: %1,\nis not a regular file.");
const char *const QBtWorkspace::NO_SELECTED = QT_TR_NOOP("Not selected");
const char *const QBtWorkspace::FILES_EQUAL =
    QT_TR_NOOP("Both files are the same.");

//*******************************************************************
// QBtWorkspace                                          CONSTRUCTOR
//*******************************************************************
QBtWorkspace::QBtWorkspace(QWidget *const in_parent)
    : QWidget(in_parent), next(new QPushButton("next")),
      ...
      openA_btn_(new QPushButton("...")), openB_btn_(new QPushButton("...")),
      pathA_cbx_(new QComboBox), pathB_cbx_(new QComboBox),
      scrollA_bar_(new QScrollBar), scrollB_bar_(new QScrollBar),
      ...
      dirA_(QDir::homePath()), dirB_(dirA_), can_scroll_(true) {
  ...
  main_layout->addWidget(previous, 2, 1);
  main_layout->addWidget(next, 2, 5);
  ...
  connect(pathA_cbx_, SIGNAL(activated(const QString &)), this,
          SLOT(lft_fpath_activated(const QString &)));
  connect(pathB_cbx_, SIGNAL(activated(const QString &)), this,
          SLOT(rgt_fpath_activated(const QString &)));
  connect(openA_btn_, SIGNAL(clicked()), this, SLOT(lft_file_selection()));
  connect(openB_btn_, SIGNAL(clicked()), this, SLOT(rgt_file_selection()));
  connect(saveA_btn_, SIGNAL(clicked()), this, SLOT(save_A()));
  connect(saveB_btn_, SIGNAL(clicked()), this, SLOT(save_B()));

  connect(next, SIGNAL(clicked()), this, SLOT(next_pair()));
  connect(previous, SIGNAL(clicked()), this, SLOT(previous_pair()));
  
  connect(browserA_, SIGNAL(scroll_request(int)), this,
          SLOT(browser_A_scrolled(int)));
  connect(browserB_, SIGNAL(scroll_request(int)), this,
          SLOT(browser_B_scrolled(int)));
  connect(browserA_, SIGNAL(update_request()), this, SLOT(update_request()));
  connect(browserB_, SIGNAL(update_request()), this, SLOT(update_request()));
...
//*******************************************************************
void QBtWorkspace::document_changed(const bool in_isA,
                                    const bool in_is_changed) {
  if (in_isA)
    saveA_btn_->setEnabled(in_is_changed);
  else
    saveB_btn_->setEnabled(in_is_changed);
...
void QBtWorkspace::save_config() const {
  QBtSettings stt;
  {
    ...
    stt.save(QBtConfig::MAIN_WINDOW_GROUP + QBtConfig::LFT_FPATH_HISTORY_KEY,
             data);
  }
  {
    QStringList data = QStringList();
    ...
    stt.save(QBtConfig::MAIN_WINDOW_GROUP + QBtConfig::RGT_FPATH_HISTORY_KEY,
             data);
  }
}
// end of save_config
...
void QBtWorkspace::restore_config() {
  ...
  if (stt.read(QBtConfig::MAIN_WINDOW_GROUP + QBtConfig::LFT_FPATH_HISTORY_KEY,
               data)) {
    dataA = data.toStringList();
  }
  if (stt.read(QBtConfig::MAIN_WINDOW_GROUP + QBtConfig::RGT_FPATH_HISTORY_KEY,
               data)) {
...
// save_A                                               PRIVATE slot
//*******************************************************************
void QBtWorkspace::save_A() { browserA_->save_file(); }
// end of save_A

//*******************************************************************
// save_B                                               PRIVATE slot
//*******************************************************************
void QBtWorkspace::save_B() { browserB_->save_file(); }
// end of save_B
...
void QBtWorkspace::browser_A_scrolled(const int in_dy) {
  if (QBtConfig::instance()->sync() && can_scroll_) {
    ...
    const int nr =
        (in_dy >= 0) ? browserA_->first_selected() : browserA_->last_selected();
    if (nr != -1) {
      const int snr = QBtDiffProcess::instance()->first_to_second(nr);
      if (snr != -1)
        browserB_->goto_line(snr ? snr : 1);
    }
    can_scroll_ = true;
  }
...
void QBtWorkspace::browser_B_scrolled(const int in_dy) {
  if (QBtConfig::instance()->sync() && can_scroll_) {
    ...
    const int nr =
        (in_dy >= 0) ? browserB_->first_selected() : browserB_->last_selected();
    if (nr != -1) {
      const int fnr = QBtDiffProcess::instance()->second_to_first(nr);
      if (fnr != -1)
        browserA_->goto_line(fnr ? fnr : 1);
    }
    can_scroll_ = true;
  }
  browserA_->numeration_update();
...
void QBtWorkspace::del_on_A() {
  ...
  if (block.isValid() && block.userData()) {
    ...
    while (block.isValid()) {
      const QBtLineData *const info =
          dynamic_cast<QBtLineData *>(block.userData());
      ...
      if (QBtShared::DELETE == opc) {
        ...
        if (di.is_valid()) {
          ...
          block = (prv_block.isValid()) ? prv_block.next()
                                        : browserA_->document()->begin();
          continue;
        }
      }
      block = block.next();
...
void QBtWorkspace::next_pair(){

  auto const file1 = browserA_->fpath();
  auto const file2 = browserB_->fpath();  
  auto const list = file2.split(QRegExp("xx"));
  auto const currentNumber = list.back().toInt();
  auto newFile = list.front() + "xx"+ QString("%1").arg(currentNumber+1, 3, 10, QChar('0'));
  rgt_read_file(newFile);
  lft_read_file(file2);
}
void QBtWorkspace::previous_pair(){

  auto const file1 = browserA_->fpath();
  auto const file2 = browserB_->fpath();  
  auto const list = file1.split(QRegExp("xx"));
  auto const currentNumber = list.back().toInt();
  auto newFile = list.front() + "xx"+ QString("%1").arg(currentNumber-1, 3, 10, QChar('0'));
  lft_read_file(newFile);
  rgt_read_file(file1);
}
